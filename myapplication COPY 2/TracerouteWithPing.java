...
public class TracerouteWithPing {
    ...
    private final TraceActivity traceActivity;
    ...
    public TracerouteWithPing(TraceActivity traceActivity) {
    ...
    private class TimeOutAsyncTask extends AsyncTask<Void, Void, Void> {
        ...
        private final ExecutePingAsyncTask task;
                ...
                public void run() {
                    if (task != null) {
                        ...
                        if (ttlTask == finishedTasks) {
                            Toast.makeText(traceActivity, "Timeout", Toast.LENGTH_SHORT).show();
    ...
    private class ExecutePingAsyncTask extends AsyncTask<Void, Void, String> {
        ...
        protected String doInBackground(Void... params) {
            if (hasConnectivity()) {
                try {
                    ...
                    latestTrace = trace;
                    Log.d(RecyclerViewAdapterTraceRt.tag, "hostname : " + hostname);
                    Log.d(RecyclerViewAdapterTraceRt.tag, "canonicalHostname : " + canonicalHostname);

                    // Store the TracerouteContainer object
                    Log.d(RecyclerViewAdapterTraceRt.tag, trace.toString());
                    ...
                    if (!ip.equals(ipToPing) || ttl == maxTtl) {
                        traceActivity.refreshList(trace);
                    }

                    return res;
                } catch (final Exception e) {
                    traceActivity.runOnUiThread(() -> onException(e));
                }
            } else {
                return traceActivity.getString(R.string.no_connectivity);
            }
            return "";
        }
        ...
        protected void onPostExecute(String result) {
            if (!isCancelled) {
                try {
                    if (!"".equals(result)) {
                        if (traceActivity.getString(R.string.no_connectivity).equals(result)) {
                            Toast.makeText(traceActivity, traceActivity.getString(R.string.no_connectivity), Toast.LENGTH_SHORT).show();
                        } else {
...
//							context.refreshList(traces);
                    ...
                    traceActivity.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            onException(e);
                        }
        ...
        private void onException(Exception e) {
            ...
            if (e instanceof IllegalArgumentException) {
                Toast.makeText(traceActivity, traceActivity.getString(R.string.no_ping), Toast.LENGTH_SHORT).show();
            } else {
                Toast.makeText(traceActivity, traceActivity.getString(R.string.error), Toast.LENGTH_SHORT).show();
            }
    ...
    public boolean hasConnectivity() {
        ConnectivityManager connectivityManager = (ConnectivityManager) traceActivity.getSystemService(Context.CONNECTIVITY_SERVICE);
